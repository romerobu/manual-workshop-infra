== Configure vault secrets - Pending to define app for testing
...
In https://github.com/romerobu/helm-infra-gitops-workshop[helm charts repository], in vault chart in main branch but also in your working branch, you can find a secret using a vault placeholder in *insert path to secret*:
...
If you take a look to the existing secret in vault-secrets namespace, as we are telling Application to use vault plugin, it is not replacing the sesitive information:

image::secret-vault.png[]

So we need to modify existing application sno-<name>-vault (argocd-apps) to use plugin:
