...
First Helm chart is operators, which deploys GitOps operator on managed clusters. If you navigate to subscription.yaml resource you will see there is a global value
...
By default any new instance create is namespace scoped and you need to give additional privileges to sa argo application controller. You can either create a custom role binging, set this gloval
env var o label any managed namespace by argo so it will create this role binding automatically.
...
Once the operator is running, we need to deploy ArgoCD instance. To make sure instance is deploying after the operator is running we use sync waves and custom health check resources.

Sync waves are defined on each resource as annotations, and they tell argo the order in which resources should be applied once the previos is already in healthy status.
You can take a look in detail to the documentation https://argo-cd.readthedocs.io/en/stable/user-guide/sync-waves/.
