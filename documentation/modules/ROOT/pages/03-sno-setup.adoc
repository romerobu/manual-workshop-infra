...
IMPORTANT: Make sure after cluster-definition, the following folder *(sno-<name>)* is named as your assigned cluster as this value will be used as a parameter for ApplicationSet.
...
You must also update managed-setup-a-<name> for your own managed cluster. Note how this application deploys to a different namespace and argo instance compared to hub-setup application:
...
NOTE: Learn more about https://argo-cd.readthedocs.io/en/stable/user-guide/application-set/[ApplicationSets]

ApplicationSet is a CR that can be used to genereate Applications to multiple cluster from multiple originis. ApplicationSet controller is installed alongside Argo CD 
...
IMPORTANT: As we did before, we should update ApplicationSet name with your lab data.
...
The second ApplicationSet bootstrap-sno-<name> deploys an Application in the argocd-infra instance on manged-cluster, called sno-setup, with configuration like: rbac, a second argocd.apps instance, namespaces and vault.
...
In the hub argocd instance, the first Helm chart is *gitops-setup*, which deploys GitOps operator on managed clusters. This chart is intented to deploy any kind of operator, even though in this case we are
...
Navigate to SNO argocd-infra instance and take a look to the *resourceCustomizations* section, as well as other configurations that we will review later on.
...
NOTE: https://argo-cd.readthedocs.io/en/stable/operator-manual/rbac/[RBAC] documentation
