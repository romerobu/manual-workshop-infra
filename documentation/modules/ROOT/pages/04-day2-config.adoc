...
that you might override by creating your own _values_.
...
- Replace *_<chartname>_* by the file name you are willing to use, it is just for testing.

[.lines_7]
[.console-input]
[source, shell,subs="+macros,+attributes"]
----
helm template . --debug -f values-<chartname>.yaml
---- 
...
Once the _values-oauth.yaml_ exists in your repository, make a push and create a new Application in the _argocd-infra_ instance to apply this operation:
...
[source, shell,subs="+macros,+attributes"]
----
cd ~/helm-infra-gitops-workshop/charts/oauth/templates/ldap
----
...
Once authentication is configured, we are going to deploy some operators. Operatos Helm charts use _range values_ so we can define as many operators as we want on _values_ section.
...
We are going to deploy *Tekton*, *Kiali*, *Jaeger*, *Service Mesh* and *Nmstate* operator. Furthermore we are going to deploy *_Service Mesh Control Plane_*, *_Service Mesh Member Roll_* instances and an example application called _Bookinfo_ for Service Mesh.
...
In this case *_Sync Waves_* and *_Health Checks_* are very important as for being able to deploy _Bookinfo_ app, operator must be installed in the first place but also _mesh_ should be configured.
...
In the first place, we are going to enable _user-defined_ projects monitoring. Then we will create an *_example-app_*, with a _Service Monitor_ and a custom _Prometheus Rule_.
...
This is an easy configuration as _Sync Waves_ are not a must because objects do not have direct dependencies and they do not affect to existing configurations.
...
Once you update the Application you want be able to create more than _4 pods_ in namespace _app_. Try to update replicas in _Deployment_ to see if _quota_ has been correctly applied by Argo.
...
NOTE: You will probably need to delete the _Network Policy_ *_deny-by-default_* in OpenShift in namespace _app_, as it already exists, Argo will not apply our latest changes. 
...
If you take a look to the configuration applied by the Application _sno-setup_ on your _SNO_  _argocd-infra_ instance where those resources have been already created as part of _bootstrapping_. Do not make any change.
...
- Change to project _vault-secrets_ -> In _Search_ bar type: _secret_
...
Replace only the plugin section:

- Click *_sno-<name>-vault_* -> then *_App details_*

IMPORTANT: Change _<your_user>_ for your GitHub user account.
...
NOTE: If you do not need to pass any plugin you can simply invoke *"plugin: {}"*.
...
Checkout to *main-day2* branch in this https://github.com/romerobu/workshop-gitops-content-deploy.git[workshop-gitops-content-deploy] repository to take a look:
...
If you navigate to the Charts folder, *_~/workshop-gitops-content-deploy/cluster-addons/day2-as/_*, you will see you are not creating objects itself but Applications. Lets test it.
